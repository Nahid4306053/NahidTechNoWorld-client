...
export default function RelatedProduct({type,id}) {
const [RelatedProducts, setRelatedProducts] = useState([]);
useEffect(() => {
  axios.get(`${import.meta.env.VITE_API_URL}/related/product/${id}?type=${type}`,{withCredentials:true}).then(res=>{
    if(res.data.data){
  ...
  return (
    <div className="container mx-auto my-20">
      ...
      <div className="grid md:grid-cols-2 grid-cols-1 lg:grid-cols-3 mt-14 gap-10">
       {RelatedProducts.length > 0 && RelatedProducts.map(ele=>{
        return (
